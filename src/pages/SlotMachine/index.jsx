...
export default function SlotMachine({ func }) {
    const nav = useNavigate();
    // TODO: להחליף את המספר אתגר והמספר קלף
    const { cardId, activeChallengeId } = useParams();
    const { data, loading, error } = useApi({ method: "get", path: `/luck/resultCasino/${activeChallengeId}/cardId/${cardId}` });
    const [winningSlots, setWinningSlots] = useState(initialSlot);
            ...
            <main className={styles.slotMachine}>
                {showWin ? <div className={styles.winContainer}>
                    ...
                    <div className={styles.btns}>
                        <Button
                            onClick={isSpinned && !showWin && !win ? () => func() : spin}
                            style={{ width: "100%", fontSize: "17px" }}
                            disabled={isSpinned && !showWin && !win ? false : isSpinned}
                            classname={isSpinned && !showWin && !win ? '' : isSpinned ? styles.inActive : ''}>
                            {isSpinned && !showWin && !win ? 'Continue' : "Spin"}
                        </Button>
                        <button className={styles.skip} onClick={() => func()}>Skip for today</button>
                    </div>
                </>}
            </main>
            <Footer />
